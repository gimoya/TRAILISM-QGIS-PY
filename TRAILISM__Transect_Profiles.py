...
class GenerateProfilesAndSlopeIntersections(QgsProcessingAlgorithm):
	...
	def processAlgorithm(self, parameters, context, feedback):
		...
		surf_top_mode = int(self.parameterAsEnum(parameters, self.SURF_TOP_CROSSFALL_MODE, context))
		surf_top_tilt_deg = int(self.parameterAsInt(parameters, self.SURF_TOP_TILT_DEG, context))
			...
			for part_idx, part in enumerate(parts):
				...
				for station in stations:
					...
					if z0 is None:
						...
						# Axis point: populate z fields with None, keep area None (areas not computed when DEM fails)
						ptA.setAttributes([station, "axis_center", "center", "edge", None, None, None, None, 0, None, None])
					...
					if should_plot:
						try:
							...
							try:
								...
								feedback.pushWarning(f"Matplotlib not available - skipping SVG generation. Error: {e}")
								continue
